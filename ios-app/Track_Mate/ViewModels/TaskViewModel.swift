...
class TaskViewModel: ObservableObject {
    ...
    private let apiBaseURL = "http://192.168.8.196:3000/api/tasks"
    ...
    func fetchTasks() {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            DispatchQueue.main.async {
                do {
                    // MongoDB'den gelen tarih formatını anlaması için özel ayar
                    let decoder = JSONDecoder()
                    decoder.dateDecodingStrategy = .iso8601
                    
                    // Gelen JSON bir dizi ([]) olduğu için [TaskItem].self kullandık
